...
func (r *readTrackingBody) Close() error {
	rdma.DPrintf("rdma debug: http transfer readTrackingBody Close()")
	r.didClose = true
...
func (t *Transport) queueForIdleConn(w *wantConn) (delivered bool) {
	rdma.DPrintf("rdma debug: transport.go queueForIdleConn() start\n")
	...
	if list, ok := t.idleConn[w.key]; ok {
		rdma.DPrintf("rdma debug: transport.go queueForIdleConn() ret list %v\n", list)
...
func (t *Transport) dial(ctx context.Context, network, addr string) (net.Conn, error) {
	if t.DialContext != nil {
		rdma.DPrintf("rdma debug: *Transport.dial() ctx %v\n", ctx)
		return t.DialContext(ctx, network, addr)
	}
	if t.Dial != nil {
		rdma.DPrintf("rdma debug: *Transport.dial() ctx %v", ctx)
	...
	rdma.DPrintf("rdma debug: *Transport.dial() zeroDialer.DialContext ctx %v", ctx)
...
func (t *Transport) getConn(treq *transportRequest, cm connectMethod) (pc *persistConn, err error) {
	rdma.DPrintf("rdma debug: transport.go getConn()")
	...
	defer func() {
		rdma.DPrintf("rdma debug: transport.go getConn() return pc %v, err %v\n", pc, err)
	...
	rdma.DPrintf("rdma debug: transport.go queueForIdleConn() return false \n")
	...
	case <-w.ready:
		...
		if w.err != nil {
			...
			case <-req.Context().Done():
				rdma.DPrintf("rdma debug: transport.go req.Context().Done() \n")
		...
		rdma.DPrintf("rdma debug: transport.go will return wantConn %v \n", w.pc)
		return w.pc, w.err
	case <-req.Cancel:
		return nil, errRequestCanceledConn
	case <-req.Context().Done():
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod) (pconn *persistConn, err error) {
	...
	if cm.scheme() == "https" && t.hasCustomTLSDialer() {
		...
		rdma.DPrintf("rdma debug: transport dialConn() dialed tcp using addr %v\n", cm.addr())
		if err != nil {
			return nil, wrapErr(err)
		}
		pconn.conn = conn
	...
	rdma.DPrintf("rdma debug: transport dialConn() will create goroutine to run readLoop and writeLoop\n")
	go pconn.readLoop()
	go pconn.writeLoop()
	return pconn, nil
}
...
func (pc *persistConn) Read(p []byte) (n int, err error) {
	rdma.DPrintf("rdma debug: persistConn Read()")
	if pc.readLimit <= 0 {
...
func (pc *persistConn) readLoop() {
	rdma.DPrintf("rdma debug: transport readLoop() start\n")
...
func (pc *persistConn) readResponse(rc requestAndChan, trace *httptrace.ClientTrace) (resp *Response, err error) {
	rdma.DPrintf("rdma debug: (pc *persistConn) readResponse() start\n")
...
func (pc *persistConn) closeLocked(err error) {
	rdma.DPrintf("rdma debug: transport closeLocked() start\n")
	...
	rdma.DPrintf("rdma debug: transport closeLocked() returning\n")
}
