...
func (cr *connReader) startBackgroundRead() {
	...
	cr.inRead = true
	rdma.DPrintf("rdma debug: startBackgroundRead() will SetReadDeadline")
	cr.conn.rwc.SetReadDeadline(time.Time{})
	go cr.backgroundRead()
}

func (cr *connReader) backgroundRead() {
	rdma.DPrintf("rdma debug: backgroundRead, cr %v\n", cr)
	...
	if ne, ok := err.(net.Error); ok && cr.aborted && ne.Timeout() {
		rdma.DPrintf("rdma debug: backgroundRead return error %v, Ignored, cr %v\n", err, cr)
	...
	cr.inRead = false
	cr.unlock()
	rdma.DPrintf("rdma debug: backgroundRead() unlock cr.lock() cr %v\n", cr)

	cr.cond.Broadcast()
}

func (cr *connReader) abortPendingRead() {
	rdma.DPrintf("rdma debug: abortPendingRead(), cr %v\n", cr)
	cr.lock()
	defer cr.unlock()
	if !cr.inRead {
		return
...
func (w *response) finishRequest() {
	rdma.DPrintf("rdma debug: finishRequest(), response %v\n", w)
	...
	rdma.DPrintf("rdma debug: finishRequest will close reqBody")
	w.reqBody.Close()
...
func (c *conn) serve(ctx context.Context) {
	...
	for {
		...
		rdma.DPrintf("rdma debug: serve() will finishRequest")
		...
		rdma.DPrintf("rdma debug: http reuse connection")
...
func (w *response) sendExpectationFailed() {
	rdma.DPrintf("rdma debug: sendExpectationFailed()")
