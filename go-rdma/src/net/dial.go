...
func (d *Dialer) DialContext(ctx context.Context, network, address string) (Conn, error) {
	rdma.DPrintf("rdma debug: DialContext()\n")
	if ctx == nil {
		panic("nil context")
	}
	...
	var c Conn
	if len(fallbacks) > 0 {
		rdma.DPrintf("rdma debug: DialContext() dialParallel\n")
		c, err = sd.dialParallel(ctx, primaries, fallbacks)
	} else {
		rdma.DPrintf("rdma debug: DialContext() dialSerial\n")
	...
	rdma.DPrintf("rdma debug: DialContext() return c %v\n", c)
	return c, nil
}
...
func (sd *sysDialer) dialSingle(ctx context.Context, ra Addr) (c Conn, err error) {
	rdma.DPrintf("rdma debug: dialSingle() ra %v\n", ra)
