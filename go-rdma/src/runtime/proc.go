...
// 在 g0 上执行 ready 方法，// 将 gp goroutine 由 _Gwaiting 改为 _Grunnable
func goready(gp *g, traceskip int) {
	systemstack(func() {
		ready(gp, traceskip, true)// 将 gp goroutine 由 _Gwaiting 改为 _Grunnable
	})
}
...
// 将传入的 goroutine gp 标记为 _Grunnable 状态，表示准备好可以运行
// 并尽快为调用 ready 的当前 goroutine 找到 P 调度运行
func ready(gp *g, traceskip int, next bool) {
	if trace.enabled {
		traceGoUnpark(gp, traceskip)
	}

	status := readgstatus(gp)// 获得 goroutine 的 read 和 write 操作状态

	// Mark runnable.
	_g_ := getg()// 获得当前 goroutine 的指针
	// acquirem() 获取当前 goroutine 对应的 m，同时 m.locks++
	...
	casgstatus(gp, _Gwaiting, _Grunnable)// 将传入的 goroutine 的状态由 _Gwaiting 更新成 _Grunnable
	// 尝试将当前 goroutine 放入当前m 对应的 P 的可运行队列；
	// 若入参 next 为true，则将当前 g 放入 _p_.runext 下一个就执行；否则放入 可运行队列的尾部
	// 若本地可运行队列已满，则将 g 放入全局队列
	runqput(_g_.m.p.ptr(), gp, next)
	wakep()// 增加一个P，并调度一个M 来运行P
	releasem(mp)// m.locks--
}
