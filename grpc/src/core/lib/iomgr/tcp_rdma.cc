...
static uint16_t def_sqsize = 1024;
static uint16_t def_rqsize = 1024;
static uint32_t def_mem = (1 << 22);
static uint32_t def_wmem = (1 << 22);
...
static void rdma_drop_uncovered_then_handle_write(void* arg, grpc_error* error) {
  if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
...
static bool grpc_tcp_have_rdata(grpc_tcp* tcp) {
  if (GRPC_TRACE_FLAG_ENABLED(grpc_rdma_trace)) {
    gpr_log(GPR_INFO, "TCP:%p rbuf_bytes_avail: %d, rbuf_free_offset: %d, rbuf_offset: %d",
...
static grpc_error* tcp_annotate_error(grpc_error* src_error, grpc_tcp* tcp) {
  return grpc_error_set_str(
      grpc_error_set_int(
          grpc_error_set_int(src_error, GRPC_ERROR_INT_FD,
...
static bool grpc_tcp_can_send(grpc_tcp* tcp) {
  if (GRPC_TRACE_FLAG_ENABLED(grpc_rdma_trace)) {
    gpr_log(GPR_INFO, "TCP:%p sqe_avail:%d, tcp->sbuf_bytes_avail:%d, sseq_no:%d, sseq_comp:%d, tcp->target_sgl[%d].length: %d",
...
static int grpc_tcp_write_data(grpc_tcp* tcp, struct ibv_sge *sgl, int nsge,
  ...
  return grpc_tcp_post_write_msg(tcp, sgl, nsge,
                                 tcp_msg_set(TCP_OP_DATA, length),
                                 flags, addr, rkey);
}
...
static void grpc_tcp_send_credits(grpc_tcp* tcp) {
  ...
  if (tcp->remote_sgl.key == 0) {
    gpr_log(GPR_INFO,"TCP:%p grpc_tcp_send_credits remote_sgl.key %d is 0, and return",tcp,tcp->remote_sgl.key);
    return;
  }
  tcp->ctrl_seqno++;
  ...
  if (tcp->rbuf_bytes_avail >= (tcp->rbuf_size >> 1)) {
    ...
    if (GRPC_TRACE_FLAG_ENABLED(grpc_rdma_trace)) {
      gpr_log(GPR_INFO,
              "TCP:%p ctrl_seqno: %d, rseq_comp: %d, rbuf_free_offset: %d, remote_sge: %d, remote sseq_comp: %d",
              tcp, tcp->ctrl_seqno, tcp->rseq_comp, tcp->rbuf_free_offset,
              tcp->remote_sge, tcp->rseq_no + tcp->rq_size);
    }

    grpc_tcp_post_write_msg(tcp, &ibsge, 1,
                            tcp_msg_set(TCP_OP_SGL,
                                        tcp->rseq_no + tcp->rq_size),
                            flags,
                            tcp->remote_sgl.addr + tcp->remote_sge *
                            sizeof(struct grpc_tcp_sge),
                            tcp->remote_sgl.key);
    ...
    if (GRPC_TRACE_FLAG_ENABLED(grpc_rdma_trace)) {
      gpr_log(GPR_INFO,
              "TCP:%p update rbuf_bytes_avail: %d rbuf_free_offset: %d",
              tcp, tcp->rbuf_bytes_avail, tcp->rbuf_free_offset);
    }
    ...
    grpc_tcp_post_msg(tcp, tcp_msg_set(TCP_OP_SGL, tcp->rseq_no +
                                       tcp->rq_size));
  }
}
...
static int grpc_tcp_poll_cq(grpc_tcp* tcp) {
  ...
  while (tcp->cm_id->recv_cq && ((ret = ibv_poll_cq(tcp->cm_id->recv_cq, 1, &wc)) > 0)) {
    if (tcp_wr_is_recv(wc.wr_id)) {
      if (wc.status != IBV_WC_SUCCESS) {
        if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
          gpr_log(GPR_INFO, "TCP:%p wc status: %d, errno: %d",
                  tcp, wc.status, errno);
        }
        continue;
      }
      ...
      case TCP_OP_SGL:
        ...
        if (GRPC_TRACE_FLAG_ENABLED(grpc_rdma_trace)) {
          gpr_log(GPR_INFO, "TCP:%p sseq_comp: %d, 0's length: %d, 1's length: %d",
                  tcp, tcp->sseq_comp, tcp->target_sgl[0].length,
      ...
      switch (tcp_msg_op(tcp_wr_data(wc.wr_id))) {
        case TCP_OP_SGL:
          tcp->ctrl_max_seqno++;
          if (GRPC_TRACE_FLAG_ENABLED(grpc_rdma_trace)) {
            gpr_log(GPR_INFO, "TCP:%p ctrl_max_seqno: %d",
                    tcp, tcp->ctrl_max_seqno);
          }
          break;
        case TCP_OP_CTRL:
          tcp->ctrl_max_seqno++;
        ...
        default:
          ...
          if (GRPC_TRACE_FLAG_ENABLED(grpc_rdma_trace)) {
            gpr_log(GPR_INFO, "TCP:%p sqe_avail: %d, sbuf_bytes_avail: %d",
                    tcp, tcp->sqe_avail, tcp->sbuf_bytes_avail);
          }
          break;
      }
      if (wc.status != IBV_WC_SUCCESS) {
...
static int grpc_tcp_process_cq(grpc_tcp* tcp, int nonblock,
  ...
  do {
    ...
    if (cb(tcp)) {
      ...
      if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
        if (ret < 0) {
          gpr_log(GPR_INFO, "TCP:%p ibv_req_notify_cq failed: %d, set cq_armed to 1",
                  tcp, errno);
        } else {
...
static bool rdma_flush(grpc_tcp* tcp, grpc_error** error) {
  ...
  for (; left; left -= xfer_size) {
    ...
    if (xfer_size <= tcp_sbuf_left(tcp)) {
      ...
      ret = grpc_tcp_write_data(tcp, tcp->ssgl, 1, xfer_size,
      ...
      grpc_tcp_copy_from_slices(tcp->sbuf, tcp->outgoing_buffer,
                                &outgoing_slice_idx,
                                &tcp->outgoing_byte_idx, tcp->ssgl[1].length);
      ret = grpc_tcp_write_data(tcp, tcp->ssgl, 2, xfer_size,
                                xfer_size <= tcp->sq_inline ?
                                IBV_SEND_INLINE : 0);
      if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
    ...
    if (ret) {
      ...
      if (errno == ENOMEM) {
        if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
          gpr_log(GPR_INFO, "TCP:%p grpc_tcp_write_data failed, errno: %d",
                  tcp, errno);
        }
        goto delayed;
      } else {
...
static void rdma_shutdown(grpc_endpoint* ep, grpc_error* why) {
  ...
  if (tcp->cm_fd && !grpc_fd_is_shutdown(tcp->cm_fd)) {
    rdma_disconnect(tcp->cm_id);
    grpc_fd_shutdown(tcp->cm_fd, why);
    if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
...
static void call_read_cb(grpc_tcp* tcp, grpc_error* error) {
  ...
  if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
    ...
    gpr_log(GPR_INFO, "TCP:%p READ length: %ld",
            tcp, tcp->incoming_buffer->length);
  }
...
static void rdma_do_read(grpc_tcp* tcp) {
  ...
  do {
    ...
    for (; left && grpc_tcp_have_rdata(tcp); left -= rsize) {
      if (left < tcp->rmsg[tcp->rmsg_head].data) {
        ...
        if (++tcp->rmsg_head == tcp->rq_size + 1)
          tcp->rmsg_head = 0;
      }
  ...
  if (total_read_bytes > 0) {
    if (total_read_bytes < tcp->incoming_buffer->length) {
      if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
        gpr_log(GPR_INFO, "TCP:%p trim incoming buffer %ld",
                tcp, tcp->incoming_buffer->length - total_read_bytes);
      }
...
static void rdma_continue_read(grpc_tcp* tcp) {
  ...
  if (tcp->incoming_buffer->length == 0 &&
    ...
    if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
      gpr_log(GPR_INFO, "TCP:%p alloc_slices, size: %ld", tcp, target_read_size);
    }
...
static void rdma_write(grpc_endpoint* ep, grpc_slice_buffer* buf,
  ...
  if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
    ...
    gpr_log(GPR_INFO, "TCP:%p WRITE length: %ld, buf size: %ld",
            tcp, buf->length, buf->count);
  }
...
int grpc_tcp_poll_fd(grpc_tcp* tcp, int events, int nonblock,
  ...
  grpc_tcp_process_cq(tcp, nonblock, test);
...
static grpc_error* grpc_tcp_init_bufs(grpc_tcp *tcp) {
  ...
  tcp->sbuf = static_cast<uint8_t*>(
    gpr_malloc_aligned(total_sbuf_size, pg_size));
  ...
  if (!tcp->target_mr) {
    error =  grpc_error_set_int(GRPC_ERROR_CREATE_FROM_STATIC_STRING(
                                  "failed to call rmda_reg_write"),
                                GRPC_ERROR_INT_ERRNO, errno);
    goto err_out2;
  }
  memset(tcp->target_buffer_list, 0, len);
  tcp->target_sgl = static_cast<volatile grpc_tcp_sge*>
    (tcp->target_buffer_list);
  total_rbuf_size = tcp->rbuf_size;
  tcp->rbuf = static_cast<uint8_t*>(
    gpr_malloc_aligned(total_rbuf_size, pg_size));
...
void grpc_tcp_format_conn_data(grpc_endpoint* ep,
  ...
  if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
    gpr_log(GPR_INFO, "TCP:%p conn version: %d, flags: %d, credits: %d",
            tcp, conn->version, conn->flags, tcp->rq_size);
    gpr_log(GPR_INFO,
            "TCP:%p conn target_sgl's addr: 0x%p, length: %d, key: %d",
            tcp, tcp->target_sgl, TCP_SGL_SIZE, tcp->target_mr->rkey);
...
void grpc_tcp_save_conn_data(grpc_endpoint* ep, grpc_tcp_conn_data* conn) {
  ...
  if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
    gpr_log(GPR_INFO,
            "TCP:%p tcp remote_sgl's addr: 0x%lx, length: %d, key: %d",
            tcp, tcp->remote_sgl.addr, tcp->remote_sgl.length,
            tcp->remote_sgl.key);
    gpr_log(GPR_INFO,
            "TCP:%p tcp target_sgl[0]'s addr: 0x%lx, length: %d, key: %d",
            tcp, tcp->target_sgl[0].addr, tcp->target_sgl[0].length,
...
grpc_error* grpc_tcp_create(grpc_endpoint** ep, grpc_fd* cm_fd,
  ...
  *ep = nullptr;
      ...
      if (0 == strcmp(args->args[i].key, GRPC_ARG_TCP_READ_CHUNK_SIZE)) {
        ...
        tcp_max_read_chunk_size =
            grpc_channel_arg_get_integer(&args->args[i], options);
                 ...
                 strcmp(args->args[i].key, GRPC_ARG_RESOURCE_QUOTA)) {
  ...
  if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
    ...
    gpr_log(GPR_INFO, "TCP:%p sbuf_size: %d, rbuf_size: %d",
            tcp, tcp->sbuf_size, tcp->rbuf_size);
    gpr_log(GPR_INFO, "TCP:%p sq_inline: %d, sq_size: %d, rq_size: %d",
            tcp, tcp->sq_inline, tcp->sq_size, tcp->rq_size);
...

  {
    ...
    int ret = rdma_create_qp(tcp->cm_id, NULL, &qp_attr);
    if (ret < 0) {
      gpr_log(GPR_ERROR, "TCP:%p grpc tcp failed to create cq, errno: %d",
              tcp, errno);
      goto err_out;
    }
    tcp->sq_inline = qp_attr.cap.max_inline_data;
    tcp->em_fd = grpc_fd_create(id->recv_cq_channel->fd, peer_string, true, tcp);
