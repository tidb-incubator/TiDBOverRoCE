...
struct pollable {
  ...
  struct epoll_event rpevents[MAX_EPOLL_EVENTS];
  grpc_fd** fds;
  int fds_sz;
  int fds_cap;
};
...
struct grpc_fd {
  grpc_fd(int fd, const char* name, bool track_err, void* data)
      : fd(fd), track_err(track_err), data(data) {
...
static grpc_fd* fd_create(int fd, const char* name, bool track_err,
  ...
  return new (new_fd) grpc_fd(fd, name, track_err, data);
}
...
static void pollable_record_fd(pollable* p, grpc_fd* fd) {
  if (p->fds_sz == p->fds_cap) {
    p->fds_cap = GPR_MAX(p->fds_sz * 2, 8);
    p->fds = static_cast<grpc_fd**>(gpr_realloc(
                                      p->fds, p->fds_cap * sizeof(*p->fds)));
  }
  p->fds[p->fds_sz++] = fd;
}

static void pollable_remove_fd(pollable* p, grpc_fd* fd) {
  int i;
  for (i = 0; i < p->fds_sz; i++) {
    if (p->fds[i] == fd) {
      break;
    }
  }
  GPR_ASSERT(i != p->fds_sz);
  for (; i < p->fds_sz; i++) {
    p->fds[i] = p->fds[i + 1];
  }
  p->fds_sz--;
...
static void fd_orphan(grpc_fd* fd, grpc_closure* on_done, int* release_fd,
  ...
  if (release_fd != nullptr) {
    ...
    if (pollable_obj != nullptr) {  // For PO_FD.
      ...
      pollable_remove_fd(pollable_obj, fd);
    }
...
static grpc_error* pollable_process_events(grpc_pollset* pollset,
  ...
  for (int i = 0; (drain || i < handle_count) &&
    ...
    struct epoll_event* ev = &pollable_obj->rpevents[n];
...
static int pollable_poll_check(pollable* p) {
  int i, cnt = 0, revents = 0, r = 0;
  for (i = 0; i < p->fds_sz; ++i) {
    grpc_fd* fd = p->fds[i];
    grpc_tcp* tcp = static_cast<grpc_tcp*>(fd->data);
    if (tcp) {
      revents = grpc_tcp_poll_fd(tcp, EPOLLIN | EPOLLOUT, 1,
                                 grpc_tcp_poll_all);
      if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
        gpr_log(GPR_INFO, "TCP: %p got event %d after poll fd",
                tcp, revents);
      }
      if (revents) {
        GPR_ASSERT(cnt < MAX_EPOLL_EVENTS);
        struct epoll_event* ev = &p->rpevents[cnt];
        ev->events = revents;
        ev->data.ptr = reinterpret_cast<void*>(reinterpret_cast<intptr_t>(fd) |
                                               (fd->track_err ? 2 : 0));
        cnt++;
      }
    } else {
  ...
  if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
    gpr_log(GPR_INFO, "pollable_poll_check: polled %d events", cnt);
  }
  return cnt;
}

static int pollable_poll_arm(pollable* p) {
  int i, revents = 0, cnt = 0;
  for (i = 0; i < p->fds_sz; ++i) {
    grpc_fd* fd = p->fds[i];
    grpc_tcp* tcp = static_cast<grpc_tcp*>(fd->data);
    if (tcp) {
      revents = grpc_tcp_poll_fd(tcp, EPOLLIN | EPOLLOUT, 0,
                                 grpc_tcp_is_cq_armed);
      if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
        gpr_log(GPR_INFO, "TCP: %p got event %d after poll fd",
                tcp, revents);
      }
  ...
  if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
    gpr_log(GPR_INFO, "pollable_poll_arm: polled %d events", cnt);
  }
  return cnt;
}

static int pollable_poll_events(pollable* p, int n) {
  int i, cnt = 0;
  for (i = 0; i < n; i++) {
    struct epoll_event* ev = &p->events[i];
    GPR_ASSERT(ev->events != 0);
    void* data_ptr = ev->data.ptr;
    if (!(1 & reinterpret_cast<intptr_t>(data_ptr))) {
      grpc_fd* fd =
        reinterpret_cast<grpc_fd*>(reinterpret_cast<intptr_t>(data_ptr) & ~2);
      grpc_tcp* tcp = static_cast<grpc_tcp*>(fd->data);
      if (tcp) {
        grpc_tcp_get_cq_event(tcp);
        ev->events = grpc_tcp_poll_fd(tcp, ev->events | EPOLLOUT, 1,
                                      grpc_tcp_poll_all);
        if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
          gpr_log(GPR_INFO, "TCP: %p pollable_poll_events got event %d",
                  tcp, ev->events);
        }
      }
    }
  ...
  if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
    gpr_log(GPR_INFO, "pollable_poll_events: polled %d(%d) events", cnt, n);
  }
  return cnt;
}
...
static grpc_error* pollable_epoll(pollable* p, grpc_millis deadline) {
  ...
  grpc_millis start_time = 0;
  uint32_t poll_time;
  int r;
  do {
    r = pollable_poll_check(p);
    if (r) {
      goto end;
    }
    if (!start_time) {
  ...
  do {
    ...
    if (r > 0) {
      p->event_count = r;
      r = pollable_poll_events(p, r);
    } else {
      if (GRPC_TRACE_FLAG_ENABLED(grpc_tcp_trace)) {
        gpr_log(GPR_INFO, "POLLABLE:%p[%s] return 0", p,
                pollable_desc(p).c_str());
      }
    }
  } while (!r && timeout);
