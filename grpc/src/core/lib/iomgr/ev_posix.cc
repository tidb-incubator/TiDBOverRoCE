...
grpc_fd* grpc_fd_create(int fd, const char* name, bool track_err, void *data) {
  GRPC_POLLING_API_TRACE("fd_create(%d, %s, %d, %p)",
                         fd, name, track_err, data);
  GRPC_FD_TRACE("fd_create(%d, %s, %d)", fd, name, track_err);
  return g_event_engine->fd_create(
      fd, name, track_err && grpc_event_engine_can_track_errors(), data);
}
